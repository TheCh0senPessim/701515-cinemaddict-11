...
const createMovieDetailsPopupTemplate = () => {
  return (
...
render(siteMainElement, createCatalogTemplate(), `beforeend`);
...
for (let i = 0; i < MOVIES_COUNT; i++) {
  render(moviesListAll, createMovieCardTemplate(), `beforeend`);
}
...
for (let i = 0; i < moviesListExtra.length; i++) {
  ...
  for (let j = 0; j < MOVIES_EXTRA_COUNT; j++) {
    render(extraFilmsContainer, createMovieCardTemplate(), `beforeend`);
  }
}
